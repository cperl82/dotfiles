...
#+begin_src emacs-lisp :var in-a-tmp-dir='nil :results value verbatim :eval no
  ...
  ;; CR cperl: You might want to come up with an automated way to run
  ;; the equivalent of `straight-thaw-versions' and `straight-check-all'
  ;; if you're not running from a temp dir to make sure you have the
  ;; same versions of the packages that your real configuration will use.
