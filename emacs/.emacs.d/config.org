       ...
       (lambda (entry)
         (let* ((path (car entry))
           ...
           (cons
            ...
            (cdr entry))))
       result))
    (when (not in-a-tmp-dir)
      ;; These need to be kept in sync with the files you're tangling
      ;; the code blocks into
      (ignore-errors
...
Currently this doesn't quite work well because this temporary directory will be missing ~escreen.el~. I should probably just update the code block above to explicitly copy the ~lisp~ directory into the temp dir.
